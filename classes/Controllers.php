...
<strong>Error !</strong> Email already Exists, please try another Email... !</div>';
      ...
      $stmt->bindValue(':email', $email);
      $stmt->bindValue(':password', SHA1($password));
      $stmt->bindValue(':status', 0); // 1 deactivate 0 active
      $result = $stmt->execute();
      if ($result) {
        $msg = '<div class="alert alert-success mt-3" id="flash-msg">
                <strong>Success !</div>';
          return $msg;
      }else{
        $msg = '<div class="alert alert-danger alert-dismissible mt-3" id="flash-msg">
  ...
  // Add New User By Admin
  public function addNewParticipant($seminar_id,$data)
  {
      $name = $data['name'];
      $email = $data['email'];
      $job = $data['job'];
      ...
      $sql = "INSERT INTO participants(seminar_id, name, email, job, phone) VALUES(:seminar_id, :name, :email, :job, :phone)";
      $stmt = $this->db->pdo->prepare($sql);
      $stmt->bindValue(':seminar_id', $seminar_id);
      $stmt->bindValue(':name', $name);
      $stmt->bindValue(':email', $email);
      ...
      {
        $msg = '<div class="alert alert-success mt-3" id="flash-msg">
                <strong>Successfully registered !</div>';
          return $msg;
      }
      else
      {
    ...
    // addSeminar
  public function addSeminar($data)
  {
    // print_r($data);
      $topic = $data['topic'];
      $date = $data['date'];
      ...
      $sql = "INSERT INTO seminars(topic, date, time, venue, banner) VALUES(:topic, :date, :time, :venue, :banner)";
      $stmt = $this->db->pdo->prepare($sql);
      $stmt->bindValue(':topic', $topic);
      $stmt->bindValue(':date', $date);
      $stmt->bindValue(':time', $time);
      $stmt->bindValue(':venue', $venue);
      $stmt->bindValue(':banner', $file_name);
      $result = $stmt->execute();

      if ($result) 
      {
        echo "<script>location.href='seminar.php';</script>";
  ...
  public function addContact($data)
  {
      ...
      $sql = "INSERT INTO contacts(email, phone, job, file) VALUES(:email, :phone, :job, :file)";
      $stmt = $this->db->pdo->prepare($sql);
      $stmt->bindValue(':email', $email);
      $stmt->bindValue(':phone', $phone);
      $stmt->bindValue(':job', $job);
      $stmt->bindValue(':file', $file_name);
      $result = $stmt->execute();
  ...
  // Select All User Method
  public function selectAllParticipant($seminarid){
    $sql = "SELECT * FROM participants WHERE seminar_id = :seminar_id";
    $stmt = $this->db->pdo->prepare($sql);
    $stmt->bindValue(':seminar_id', $seminarid);
    $stmt->execute();
   ...
   // Select All User Method
   public function selectAllSeminars(){
   ...
   // Select All User Method
   public function getSeminarInfoById($seminarid){
    $sql = "SELECT * FROM seminars WHERE id = :id LIMIT 1";
    $stmt = $this->db->pdo->prepare($sql);
    $stmt->bindValue(':id', $seminarid);
    $stmt->execute();
  ...
  public function CheckActiveUser($email){
    $sql = "SELECT * FROM users WHERE email = :email and status = :status LIMIT 1";
    $stmt = $this->db->pdo->prepare($sql);
    $stmt->bindValue(':email', $email);
    ...
    public function userDeactiveByAdmin($deactive){
      $sql = "UPDATE users SET
       ...
       $result =   $stmt->execute();
        ...
        if ($result) {
          echo "<script>location.href='manage-admin.php';</script>";
          Session::set('msg', '<div class="alert alert-success alert-dismissible mt-3" id="flash-msg">
          <a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a>
